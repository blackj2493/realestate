...
/**
 * Apply Migration 037 (enable RLS on all public tables) via the Session pooler.
 *
 ...
 * table that lacks it. See supabase/migrations/037_enable_rls_all_public_tables.sql.
 *
...
async function applyMigration() {
  console.log('\n🔧 Migration 037: enable RLS on all public tables (security fix)');
  ...
  try {
    ...
    const sql = fs.readFileSync(
      path.join(__dirname, '../../supabase/migrations/037_enable_rls_all_public_tables.sql'),
      'utf-8'
    );
    ...
    console.log('✅ Migration 037 complete — every public table now has RLS enabled.');
  } finally {
    await client.end();
  }
}
