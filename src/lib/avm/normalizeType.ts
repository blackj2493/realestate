...
/** Which geography a trained matrix cohort is keyed on. Finest first. */
export type CohortRung = 'community' | 'fsa' | 'city';

/** A cohort key together with the rung it belongs to. */
export interface CohortRungKey {
  rung: CohortRung;
  key: string;
}

/**
 * The matrix-cohort keys a sale or a subject can be matched on, FINEST FIRST.
 *
 * WHY THIS EXISTS. Until 2026-08-29 a cohort was keyed on `city_region` alone, and every
 * consumer dropped any row without one. That silently removed whole markets from the model:
 * Waterloo Region and Brantford ship a blank CityRegion on both the sold and the active
 * side, so 10,681 sales never reached the trainer or the trend job — and Kitchener still
 * returned an AVM value for 1,291 of 1,292 actives, none of which ever reached HIGH
 * confidence. A missing INPUT, not a failed output.
 *
 * Chatham-Kent proved the cause was the KEY, not that one feed: its CityRegion is populated,
 * but 200 sales split across 26 communities means no community ever trains a matrix, and the
 * same guard dropped it.
 *
 * The ladder uses the region's OWN sales at progressively coarser resolution. Proximity to a
 * neighbouring community was considered and rejected: the gap is a whole contiguous region,
 * so the nearest trained community is in another market (Guelph, Hamilton, Halton) and
 * borrowing its premium structure would be wrong in a way nothing could see.
 *
 * ONE function, shared by the trainer, the trend/offset job and the live matrix lookup. A
 * second copy of this ladder is the drift this codebase keeps paying for.
 */
export function cohortRungKeys(
  cityRegion: string | null | undefined,
  postalCode: string | null | undefined,
  city: string | null | undefined
): CohortRungKey[] {
  const out: CohortRungKey[] = [];
  const cr = (cityRegion ?? '').trim();
  if (cr) out.push({ rung: 'community', key: cr });
  const fsa = fsaOf(postalCode);
  if (fsa) out.push({ rung: 'fsa', key: fsa });
  const c = (city ?? '').trim();
  if (c) out.push({ rung: 'city', key: c });
  return out;
}

/** A rung together with every spelling its key can be stored under. */
export interface CohortRungLookupKey {
  rung: CohortRung;
  keys: string[];
}
 ...
 * to themselves. Pass null for postalCode and city to search the community rung only.
 *
