...
/** All three leaderboards in one pass so the client's metric toggle needs no re-fetch. */
...
export function hasUsableHeat(stats: HeatStats): boolean {
  return [stats.cap, stats.dom, stats.drop].some((rows) => rows.length >= MIN_QUALIFYING_COMMUNITIES);
}
