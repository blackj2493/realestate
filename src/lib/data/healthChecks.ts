...
/**
 * Cities whose city-level AVM trend is missing for a KNOWN structural reason, with WHY.
 *
 * Same contract as KNOWN_GAPS: an entry keeps a real gap VISIBLE rather than silencing it,
 * and a city that starts producing trend rows is reported as "trend-gap-resolved" so this
 * list cannot rot. Remove an entry the moment its cause is fixed.
 *
 * Every entry traces to ONE cause. `refresh-avm-trend-offset.ts` skips any sold row whose
 * (city_region × sub-type) has no `avm_multiplier_matrix` cohort — and that skip also
 * removes the row from the CITY trend g(city, sub, period), which never needed a community
 * key at all. There are two routes in:
 *
 *   1. the sold feed ships a blank CityRegion — all of Waterloo Region and Brantford, 93%
 *      of the 10,681 dropped sales;
...
export interface CityTrendCoverage {
  city: string;
...
/**
 * A city carrying real inventory with NO row in `avm_trend_index` at all.
 *
 * THE FAILURE THIS REPLAYS. On 2026-08-29 the AVM priced 1,292 of 1,292 active Kitchener
 * listings and returned a value for 1,291 of them, so nothing looked broken. But Kitchener
 * had 2,020 Detached sales in the trailing window and ZERO trend rows — every one of its
 * sales was dropped before aggregation. The estimates were real numbers computed without a
 * time trend, and the only visible trace was that no Waterloo-Region listing ever reached
 * HIGH confidence while a third of the book did.
 *
 * That is the silent-null shape: not an outage, a quietly missing INPUT. An output-only
 * check cannot see it, because the output is still a plausible number.
 *
 * Severity is deliberate. An unlisted city is an ERROR — a model input vanished and nobody
 * decided that. A city in KNOWN_TREND_GAPS is a WARN, so a documented, already-scoped gap
 * stays visible every night without turning the run red and masking every other check (the
 * failure mode that hid a broken canary for six days in August 2026).
 */
export function checkCityTrendCoverage(
  rows: CityTrendCoverage[],
  knownGaps: Record<string, string> = KNOWN_TREND_GAPS
): Problem[] {
  const out: Problem[] = [];
  if (rows.length === 0) {
    return [
      {
        ...
        detail:
          "city_trend_coverage returned no cities — the RPC is empty or unapplied, so the invariant is unchecked",
      },
    ];
  }
  ...
  for (const r of rows) {
    ...
    if (r.trendRows > 0) {
      if (key in knownGaps) {
        out.push({
          severity: "info",
          check: "trend-gap-resolved",
          detail: `${r.city} now has ${r.trendRows} trend row(s) — remove it from KNOWN_TREND_GAPS`,
        });
      }
      continue;
    }
    ...
    if (key in knownGaps) {
      out.push({
        severity: "warn",
        check: "city-trend-coverage",
        detail: `${r.city}: ${r.activeListings} active listings, no city trend rows — known: ${knownGaps[key]}`,
      });
    } else {
      out.push({
        severity: "error",
        check: "city-trend-coverage",
        detail:
          `${r.city}: ${r.activeListings} active listings and NO rows in avm_trend_index — ` +
          "the AVM is pricing them with no city time trend, and still returning plausible numbers",
      });
    }
  }
  ...
  for (const key of Object.keys(knownGaps)) {
    if (!evaluated.has(key)) {
      out.push({
        ...
        check: "city-trend-coverage",
        detail:
          `KNOWN_TREND_GAPS entry "${key}" was never evaluated — the city was renamed, or its ` +
          "inventory fell below the reporting threshold",
      });
    }
  }
