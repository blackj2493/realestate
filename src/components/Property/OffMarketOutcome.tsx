...
/**
 * OffMarketOutcome — the decoded "how this played out" strip for a sold or
 * off-market (delisted) listing.
 *
 * Once a listing leaves the market the rest of the page hides its two most
 * decoded signals — the relisting-adjusted True DOM and the Deal Score are
 * surfaced ONLY for active listings (page.tsx). For a sold/off-market home those
 * are exactly the retrospective questions a landing visitor has ("how long did it
 * really take? was it priced as a good deal?"), so we surface them here, and
 * bridge to the address's full Home Pulse dossier (street pulse, sold history,
 * "if listed today", nearby active — all already built at /address).
 *
 ...
 * anonymous DOM here — anon sees a locked teaser and a free sign-in. The address
 * bridge is a public link, shown to everyone.
 */
export default function OffMarketOutcome({
  ...
  const headline = kind === "sold" ? "How this one played out" : "Before it came off the market";
  if (!hasDealScore && !addressHref) return null;
  ...
  return (
    <div className="mt-4 rounded-xl border border-cyan-500/30 bg-cyan-500/[0.04] p-4">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        {/* Deal Score — the retrospective "was it a good deal at its ask?". */}
        {hasDealScore && (
          <div className="rounded-lg border border-border bg-card/50 px-3 py-2.5">
            <p className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
      ...
      {!isAuthed && hasDealScore && (
        ...
        >
          ...
          {kind === "sold" ? "See how it sold — free" : "Unlock the full history — free"}
        </Link>
      )}
    </div>
  );
