...
import { getFlagsNearPoint, type AddressFlag } from "@/lib/address/flagsNearPoint";
...
function NearbyCard({ l }: { l: NearbyListing }) {
  return (
    <Link
      ...
      className="group overflow-hidden rounded-lg border border-border bg-card/40 transition-colors hover:border-cyan-500/40"
    >
...
export default async function AddressProfileView({
  ...
  const [nearby, flags] = await Promise.all([
    ...
    lat !== null && lng !== null ? getFlagsNearPoint(lat, lng) : Promise.resolve([] as AddressFlag[]),
  ]);
  ...
  return (
    <main className="min-h-app bg-background text-foreground">
      ...
      <div className="mx-auto max-w-[900px] px-4 py-8">
        ...
        {/* ── Things to Know: open-data geo flags (hidden until RPC applied / none found). ── */}
        {flags.length > 0 && (
          <section className="mb-6">
