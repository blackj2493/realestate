    ...
    <div className="min-h-app flex flex-col bg-slate-950 text-slate-200">
      ...
      <header className="border-b border-slate-800 px-4 py-3">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          ...
          <nav className="flex gap-1 text-xs uppercase tracking-wider text-slate-400">
            <Link
              href="/terms"
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
        <h1 className="text-2xl font-bold text-slate-100">Glossary</h1>
        ...
        <dl className="mt-8 space-y-6">
          {ENTRIES.map((e) => (
            <div key={e.term} className="border-l-2 border-cyan-700 pl-3">
              <dt className="text-lg font-semibold text-slate-100">{e.term}</dt>
              <dd className="mt-1 space-y-1.5">
                <p className="text-sm leading-relaxed text-slate-300">{e.short}</p>
                {e.formula && (
                  <p className="font-mono text-xs leading-relaxed text-cyan-300/90">{e.formula}</p>
                )}
                {e.context && (
                  <p className="text-xs leading-relaxed text-slate-500">{e.context}</p>
                )}
              </dd>
            </div>
          ))}
      ...
      {/* ── Footer ──────────────────────────────────────────────────── */}
      <footer className="border-t border-slate-800 pb-safe pt-6 text-center text-xs text-slate-400">
