-- Migration 031: De-fake the Region Scorecard cap rate (CLAUDE.md §3B, §4, §10)
-- Purpose: aggregate the REAL IDX-derived cap_rate_est (financialMetrics: IDX-lease
--          asking-rent NOI / IDX list price) instead of the fabricated
--          extrapolated_cap_rate, with the render-time sanity band [1,15]% applied
--          server-side so tier-fallback outliers don't skew median/avg/top. Part of the
--          terminal de-fake (spec docs/superpowers/specs/2026-06-06-defake-terminal-design.md §6.2).
--
-- INSTANT DDL ONLY (editor-safe): add the column + CREATE OR REPLACE the RPC (signature
-- unchanged vs 027 → no DROP needed). The heavy half — backfilling cap_rate_est from the
-- Typesense `properties` docs over the active set — lives in scripts/admin/backfill031.ts
-- (Session-pooler, batched). The ETL (transformer.ts) writes cap_rate_est on every sync so
-- new/updated rows stay fresh. Additive & non-destructive; raw_vow_sold NOT touched (§12).
--
-- The fabricated extrapolated_cap_rate column is retired with ExtrapolatedCapRateEngine
-- (spec §9); until then both columns coexist and the RPC reads only the real one.
--
-- Run: npx tsx scripts/admin/applyMigration031.ts   (or paste this file into the SQL editor)
--      then: npx tsx scripts/admin/backfill031.ts --apply
...
-- RPC: aggregate cap_rate_est within the §4 sanity band [1,15]% (CAP_RATE_BAND). Out-of-band
-- values are tier-fallback artifacts (a luxury home handed a coarse city rent, a cheap unit a
-- too-high comp), so they are EXCLUDED from the stats — not merely >0 as the 027 version did.
-- Signature identical to 027, so CREATE OR REPLACE (no DROP/overload churn).
